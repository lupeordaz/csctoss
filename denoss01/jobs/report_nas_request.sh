...
LOGFILE=/home/postgres/dba/logs/report_nas_request.`date '+%Y%m%d'`
TMPFILE=/home/postgres/dba/logs/report_nas_request.tmp
ERRFILE=/home/postgres/dba/logs/report_nas_request.err
...
EMAILTO=jobrey@cctus.com,tstovicek@cctus.com,gordaz@cctus.com,crichardson@cctus.com
...
echo "[Report Acct Nas-Request  <${REPORT_FROM} -> ${REPORT_TO}>(MST)]" > $TMPFILE
echo "                                               " >> $TMPFILE
...
if [ `grep "num_of_nas_request" $TMPFILE | wc -l` -ne 1 ]; then
  cat $TMPFILE >> $LOGFILE
  cat $TMPFILE | mail -s "`hostname` report NAS-Request FAILED!" ${ERREMAIL}
fi

if [ `grep "(0 rows)" $TMPFILE | wc -l` -ne 1 ]; then
  cat $TMPFILE | mail -s "Report NAS-Request." ${EMAILTO}
fi

echo "END   TIMESTAMP: `date '+%Y%m%d%H%M%S'`" >> $LOGFILE
echo "------------------------end report NAS-Request------------------------" >> $LOGFILE
