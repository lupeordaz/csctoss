...
if [ $# != 5 ]; then
   echo "Usage: ./manual_line_creation_vzw_sprint.sh [ESN HEX] [SALES ORDER] [BILLING ENTITY] [GROUPNAME/VRF] [STATIC IP <FALSE/TRUE>]"
   echo ""
   echo "Ex: ./manual_line_creation_vzw_sprint.sh A000001F54D5CD SO-8867 ACFN SERVICE-acfn TRUE"
   echo ""
...
#
#Check if the carrier is not USCC
#
...
if [ "${CARRIER}" = "USCC" ]; then
    ...
    echo ""
    echo "Device is USCC carrier so exiting the script......"
    echo ""
    exit 1;
fi
...
PGPASSWORD=owner psql -h atloss01 -p 5450 -d csctoss -U csctoss_owner \
            -c "SELECT public.set_change_log_staff_id(251); SELECT * FROM ops_api_assign('$ESNHEX', '$SALESORDER', $BEID, '$VRF', '$SIP')"
