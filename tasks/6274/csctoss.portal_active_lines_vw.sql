...
SELECT line.line_id
      ...
      , CASE WHEN (line.radius_username IS NOT NULL) 
             THEN (COALESCE((SELECT radreply.value 
             	            ...
             	            ), ('N/A'::text)::character varying))::text 
             ELSE 'IP Not Avalible'::text 
        END AS static_ip_address
      , be.name
      , (SELECT ops_get_connection_status.ops_get_connection_status 
      	   FROM ops_get_connection_status(line.radius_username)) AS connection_status
      , COALESCE((SELECT (max(master_radacct.acctstarttime))::text AS max 
      	           FROM master_radacct 
      	          WHERE ((master_radacct.username)::text = line.radius_username) 
      	          GROUP BY master_radacct.username), 'No connections in last 3 months'::text
      ...
      , CASE WHEN ((ew.warranty_end_date)::date >= ('now'::text)::date) 
             ...
             ELSE 'Out of warranty'::text END AS warranty_status
      ...
      , (SELECT CASE WHEN ((SELECT count(*) AS count 
                     ...
                     THEN 'NO'::text 
                     ELSE CASE WHEN ((SELECT count(*) AS count 
      ...
      , (SELECT ops_get_firmware_status.ops_get_firmware_status 
      	   FROM ops_get_firmware_status((SELECT soup_device_stats_table.firmware 
      	   	                               FROM soup_device_stats_table 
      	   	                              WHERE (soup_device_stats_table.serial_number = (SELECT unique_identifier.value 
      	   	                              	                                                FROM unique_identifier 
      	   	                              	                                               WHERE ((unique_identifier.unique_identifier_type = 'SERIAL NUMBER'::text) 
  					...
  					(
  						(
  							(
  								(
  									(
  										(line JOIN billing_entity be ON ((line.billing_entity_id = be.billing_entity_id))) 
