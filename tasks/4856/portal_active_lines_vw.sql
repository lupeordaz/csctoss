...
SELECT line.line_id
      ...
      , ( SELECT unique_identifier.value 
      	   ...
      	   WHERE (
      	   			(unique_identifier.unique_identifier_type = 'ESN HEX'::text) 
      ...
      , (SELECT unique_identifier.value 
      	   FROM unique_identifier 
      	  WHERE (
      	  			(unique_identifier.unique_identifier_type = 'SERIAL NUMBER'::text) 
      	  			AND (unique_identifier.equipment_id = lieq.equipment_id)
      	  		)
      	) AS serial_number
      ...
      , (SELECT soup_cellsignal.cellsignal 
      	  ...
      	  ORDER BY soup_cellsignal."timestamp" DESC LIMIT 1
      	) AS cellsignal
      , CASE WHEN (line.radius_username IS NOT NULL) 
             THEN (COALESCE(
             					(SELECT radreply.value 
             	    	           FROM radreply 
             	        	      WHERE (
             	              			((radreply.attribute)::text = 'Framed-IP-Address'::text) 
      ...
      , be.name
      , (SELECT ops_get_connection_status.ops_get_connection_status 
      	   FROM ops_get_connection_status(line.radius_username)
      	) AS connection_status
      , COALESCE(
      				(SELECT (max(master_radacct.acctstarttime))::text AS max 
      	               FROM master_radacct 
      	              WHERE ((master_radacct.username)::text = line.radius_username) 
      	              GROUP BY master_radacct.username
      	            ), 'No connections in last 3 months'::text
      	        ) AS last_connected_timestamp
      ...
      , CASE WHEN ((ew.warranty_end_date)::date >= ('now'::text)::date) 
             	THEN 'In warranty'::text 
             ELSE 'Out of warranty'::text 
      ...
      , (SELECT ops_get_config_status.ops_get_config_status 
      	   FROM ops_get_config_status(sci.config_name)
      	) AS config_status
      , (SELECT ops_get_firmware_status.ops_get_firmware_status 
      	   ...
      	   (
      	   	    (SELECT soup_device_stats_table.firmware 
      	   	       FROM soup_device_stats_table 
      	   	      WHERE (soup_device_stats_table.serial_number = 
      	   	      			(SELECT unique_identifier.value 
      	   	      			   FROM unique_identifier 
      					...
      					(
      						(
      							(
      								(
      									(
      										line JOIN billing_entity be ON (
      											(line.billing_entity_id = be.billing_entity_id)
      											)
      									) LEFT JOIN plan ON (
      									  (line.line_id = plan.line_id)
      									)
      								) LEFT JOIN product prd ON 
      								(
      										(plan.product_id = prd.product_id)
      								)		
      							) LEFT JOIN line_equipment lieq ON ((line.line_id = lieq.line_id))
      						) LEFT JOIN equipment eq ON ((lieq.equipment_id = eq.equipment_id))
      					) LEFT JOIN equipment_model em ON ((eq.equipment_model_id = em.equipment_model_id))
      				) LEFT JOIN location_labels liloc ON ((line.line_id = liloc.line_id))
      			) LEFT JOIN usergroup ug ON 
      				(
		      			(
		      				(
		      					(ug.username)::text = line.radius_username
		      				) AND ((ug.groupname)::text = 'userdisconnected'::text)
		      			)
		      		)
      		) LEFT JOIN equipment_warranty ew ON ((eq.equipment_id = ew.equipment_id))
      	) LEFT JOIN soup_config_info sci ON ((sci.equipment_id = eq.equipment_id))
      ) 
 WHERE 
      (
      	(
      ...
      AND (line.radius_username IS NOT NULL)
      );
