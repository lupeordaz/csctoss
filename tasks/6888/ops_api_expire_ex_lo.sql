...
BEGIN
  ...
  ELSE
    ...
    IF NOT FOUND THEN
      var_return_row.result_code := false;
      var_return_row.error_message := 'ESN HEX value does not exist.';
      RETURN NEXT var_return_row;
      RETURN;
    END IF;
  END IF;
  ...
  IF NOT FOUND THEN
    var_return_row.result_code := false;
    var_return_row.error_message := 'Line Id does not exist for the device.';
    RETURN NEXT var_return_row;
    RETURN;
  END IF;

  -- Retrieve radius_username from line table using line_id
