...
DROP FUNCTION plsh_test_lo(text);
...
ALTER TABLE radreply
	DROP CONSTRAINT radreply_operator_fk;

ALTER TABLE radcheck
	DROP CONSTRAINT radcheck_operator_fk;

ALTER TABLE radgroupcheck
	DROP CONSTRAINT radgroupcheck_operator_fk;

ALTER TABLE radgroupreply
	DROP CONSTRAINT radgroupreply_operator;

DROP TABLE dba_voda_equipment;

DROP TABLE temp_esnhex;

DROP TABLE temp_usernames;
...
CREATE OR REPLACE FUNCTION fetch_portal_conn() RETURNS text
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION ops_api_user_restore(text) RETURNS SETOF ops_api_restore_retval
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
CREATE OR REPLACE FUNCTION ops_api_user_restore(text, text) RETURNS SETOF ops_api_restore_retval
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
CREATE OR REPLACE FUNCTION ops_api_user_suspend(text) RETURNS SETOF ops_api_suspend_retval
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
CREATE OR REPLACE FUNCTION ops_api_user_suspend(text, text) RETURNS SETOF ops_api_suspend_retval
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
CREATE OR REPLACE FUNCTION rma_device_info(text) RETURNS SETOF rma_device_info_retval
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
CREATE OR REPLACE FUNCTION static_ip_radreply_sync() RETURNS SETOF text
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION term_request_by_hour_new(boolean) RETURNS SETOF hourly_report_record_type
...
SET search_path = rtnew, pg_catalog;
...
DROP SEQUENCE acl_id_seq;

DROP SEQUENCE attachments_id_seq;

DROP SEQUENCE attributes_id_seq;

DROP SEQUENCE cachedgroupmembers_id_seq;

DROP SEQUENCE customfields_id_seq;

DROP SEQUENCE customfieldvalues_id_seq;

DROP SEQUENCE groupmembers_id_seq;

DROP SEQUENCE groups_id_seq;

DROP SEQUENCE links_id_seq;

DROP SEQUENCE objectcustomfields_id_s;

DROP SEQUENCE objectcustomfieldvalues_id_s;

DROP SEQUENCE principals_id_seq;

DROP SEQUENCE queues_id_seq;

DROP SEQUENCE scripactions_id_seq;

DROP SEQUENCE scripconditions_id_seq;
