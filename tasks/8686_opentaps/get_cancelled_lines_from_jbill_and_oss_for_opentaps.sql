...
\o /opt/scripts/jbill_oss_cancelled_lines.csv
...
--WHERE 1 = 1
--AND pl.archived_date IS NOT NULL
--AND pl.archived_date BETWEEN (TO_CHAR((current_date - ''1 month''::INTERVAL), ''YYYY-MM-01'')::date) AND (TO_CHAR(current_date, ''YYYY-MM-01'')::date - ''1 day''::INTERVAL)
--AND pl.id = (SELECT MAX(id) FROM prov_line WHERE pl.line_id = line_id AND archived_date IS NOT NULL GROUP BY line_id)
