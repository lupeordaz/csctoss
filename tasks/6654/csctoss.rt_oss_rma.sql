...
CREATE OR REPLACE FUNCTION rt_oss_rma(text, text, text) RETURNS oss_rma_retval
    LANGUAGE plpgsql
    AS $_$
declare
--
...
v_rma_so_num            text;
v_errmsg                text;
v_warr_start_date       date;
v_warr_end_date         date;
...
--
    ...
    v_errmsg:='Original ESN must have a warranty period associated to the equipment';
    SELECT warranty_start_date
          ,warranty_end_date
      INTO v_warr_start_date
          ,v_warr_end_date
      FROM equipment_warranty
     WHERE equipment_id = v_oequipid;
...
--         INSERT into line_equipment
    ...
    v_errmsg:='Insert/update failed for equipment_warranty';
    RAISE NOTICE 'INSERT/UPDATE equipment_warranty: v_nequipid: %, start_date=%, model id=%'
                 , v_nequipid, v_warr_start_date, v_new_mod_ext_id;
    ...
    THEN
        ...
        values
              (v_nequipid,v_warr_start_date,v_warr_end_date);
        ...
        END IF;
    ELSE
        UPDATE equipment_warranty 
           SET warranty_start_date = v_warr_start_date 
              ,warranty_end_date = v_warr_end_date
          WHERE 1=1
            AND equipment_id = v_nequipid;
    ...
    RAISE NOTICE '[rt_oss_rma] Insert/Update for equipment_warranty succeeded. Move on to static IP handling.';
