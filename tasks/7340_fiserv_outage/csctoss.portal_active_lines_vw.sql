...
SELECT line.line_id
      ...
      , prd.product_code, plan."comment" AS sales_order_number
      ...
      , CASE WHEN (line.radius_username IS NOT NULL) 
             THEN (COALESCE((SELECT radreply.value FROM radreply 
             	              WHERE (((radreply.attribute)::text = 'Framed-IP-Address'::text) 
             	              	     AND ((radreply.username)::text = line.radius_username))), ('N/A'::text)::character varying))::text ELSE 'IP Not Avalible'::text 
